...
java -jar wad.jar ~/.wad-dropins/ &

echo "--> Running openliberty dev-environment v${SERVER_VERSION} using $s{JDK_VERSION} on docker"
docker run --rm \
    ...
    -v ~/.wad-dropins/:/opt/liberty/wlp/usr/servers/defaultServer/dropins/ \
    --name ${CONTAINER_NAME} \
